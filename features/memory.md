...
Extraction writes a staging run, validates the source revision, grounds every fact to a canonical source chunk, then activates the complete run atomically. The previous run stays searchable if extraction fails. Background indexing jobs persist across restarts, retry transient failures, and move exhausted jobs to a dead-letter state.
...
![A Cynosure memory knowledge graph](../img/memory-graph.png)
...
- **Knowledge Extraction Model** creates grounded entity mentions and assertions during explicit knowledge indexing. The Relationships and Visual Graph screens read those records directly.
...
**Reset Data → Knowledge Graph** clears the complete governed knowledge projection, its semantic graph index, and knowledge-index jobs while preserving source files and ordinary chunk vectors. Documents are marked as needing knowledge extraction and can be rebuilt from their source afterward.
